...
export default {
  ...
  cruds() {
    return CRUD({ title: '部门', url: 'dept', crudMethod: { ...crudDept }})
  },
  ...
  methods: {
    ...
    getDepts() {
      get('dept',{ enabled: true }).then(res => {
        this.depts = res.data.content.map(function(obj) {
          obj.label = obj.name
          if (obj.hasChildrenData) {
            obj.children = null
          }
          return obj
        })
    ...
    loadDepts({ action, parentNode, callback }) {
      if (action === LOAD_CHILDREN_OPTIONS) {
        get('dept',{ enabled: true, pid: parentNode.id }).then(res => {
    ...
    [CRUD.HOOK.afterValidateCU]() {
      if (this.form.pid !== null && this.form.pid === this.form.id) {
        this.$baseMessage('上级部门不能为空','warn')
        return false
      }
    ...
    changeEnabled(data, val) {
      this.$confirm('此操作将 "' + this.dict.label.dept_status[val] + '" ' + data.name + '部门, 是否继续？', '提示', {
        ...
        crudDept.edit(data).then(res => {
          this.$baseNotify(this.dict.label.dept_status[val] + '成功','操作提示', CRUD.NOTIFICATION_TYPE.SUCCESS)
        }).catch(err => {
