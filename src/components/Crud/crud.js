  ...
  const methods = {
    ...
    refresh(callback) {
      ...
      return new Promise((resolve, reject) => {
        ...
        let search = crud.getQueryParams()

        let sort = search.sort
        delete search.sort
        if (sort !== '') {
          const sortArr = sort[0].split(',')
          search['order'] = sortArr[1]
          search['orderBy'] = sortArr[0]
        }
        get(crud.url, search)
          .then((data) => {
    ...
    doDelete(data) {
      ...
      return crud.crudMethod
        ...
        .then(() => {
          ...
          if (!callVmHook(crud, CRUD.HOOK.afterDelete, data)) {
            return
          }
          crud.refresh()
        })
    ...
    selectChange(selection, row) {
      ...
      let isCheck = selection.find((val) => {
    ...
    selectionChangeHandler(val) {
      ...
      if (tempData.length !== 0) {
        for (let i = 0; i < tempData.length; i++) {
          // eslint-disable-next-line prefer-const
          let index = crud.selections.indexOf(tempData[i])
          crud.selections.splice(index, 1)
          tempData = []
        }
      }
    },
    ...
    notify(title, type = CRUD.NOTIFICATION_TYPE.INFO) {
      crud.vms[0].vm.$baseMessage(title,type)
    },
